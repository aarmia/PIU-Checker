...
# 추가된 전체 곡 리스트 조회 함수
def get_full_song_list(level: int) -> Dict[str, List[Tuple[str, str]]]:
    """
    level에 해당하는 전체 곡 리스트를
    ...
    try:
      ...
      ORDER BY mode, name;
        ...
        result: Dict[str, List[Tuple[str, str]]] = {"single": [], "double": []}
        for mode, name, img_url in rows:
            if mode in result:
                result[mode].append((name, img_url))
